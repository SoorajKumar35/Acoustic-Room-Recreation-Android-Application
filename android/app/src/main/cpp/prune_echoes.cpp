...
MatrixXd get_lrs_output(MatrixXd A, MatrixXd b)
{
    ...
    ofstream input_txt;
    input_txt.open("/sdcard/input.txt");
    input_txt << "cube\n";
    ...
    {
        if(b(row) - int(b(row)) == 0 && A(row, 0)-int(A(row, 0)) == 0 && A(row, 1)-int(A(row, 1)) == 0 && A(row, 2)-int(A(row, 2)) == 0)
        {
    ...
    // TESTING INPUT FILE
    ifstream test_txt;
    test_txt.open("/sdcard/input.txt");
    string input_line;
    while(getline(test_txt, input_line))
    {
        ss << "hi" << endl;
    }
    test_txt.close();
    ...
    create_output_txt.open("/sdcard/output.txt");
    create_output_txt.close();

    char *argv[3];
    argv[0] =  NULL;
    argv[1] = "/sdcard/input.txt";
    argv[2] = "/sdcard/output.txt";
    lrs_main(3, argv);

    //Read from the output file
    ifstream output_txt;
    output_txt.open("/sdcard/output.txt");
    string output_line;
    int V_count = 0;
